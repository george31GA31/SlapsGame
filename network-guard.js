...
(function(root){
 const hostOnly=new Set(['ROUND_START','SYNC','HOST_COUNTDOWN','REVEAL_PRELOAD','REVEAL_SHOW','MOVE_APPLY','MOVE_REJECT','SLAP_UPDATE','PENALTY_UPDATE','ROUND_OVER','MATCH_OVER','BORROWED_START','CYCLE_RESET']);
 const known=new Set([...hostOnly,'HANDSHAKE','CARD_LAYOUT','READY','DRAG','MOVE_REQ','OPPONENT_REJECT','OPPONENT_FLIP','SLAP_REQ','CONCESSION_TOKEN','CONCESSION_REQ','CONCESSION_RESULT','OPPONENT_LEFT','REMATCH_REQ','REMATCH_YES','REMATCH_NO']);
 ...
 function valid(msg,isHost){
  ...
  if(!isHost&&['MOVE_REQ','SLAP_REQ'].includes(msg.type))return false;
  ...
  if(msg.type==='OPPONENT_FLIP')return str(msg.cardId);
  const fields={ROUND_START:'state',SYNC:'state',REVEAL_PRELOAD:'result',MOVE_APPLY:'apply',MOVE_REJECT:'reject',OPPONENT_REJECT:'card'};
