...
test('20-player round robin has 380 fixtures and 38 per player',()=>{const ids=Array.from({length:20},(_,i)=>'p'+i),ms=fixtures(ids);assert.equal(ms.length,380);assert.equal(new Set(ms.map(m=>m.players.slice().sort().join(':'))).size,190);for(const id of ids)assert.equal(ms.filter(m=>m.players.includes(id)).length,38);for(let d=1;d<=19;d++)assert.equal(new Set(ms.filter(m=>m.round===d).flatMap(m=>m.players)).size,20);});
test('odd-sized leagues have no self matches, no duplicates and fair byes',()=>{for(const n of [3,5,19]){const ms=fixtures(Array.from({length:n},(_,i)=>'p'+i));assert.equal(ms.length,n*(n-1));assert.ok(ms.every(m=>m.players[0]!==m.players[1]));}});
test('eight-player knockout completes seven matches and names one champion',()=>{let r=reduce(lobby(),'p0','P',{type:'start'},1000);for(const id of ['R1-0','R1-1','R1-2','R1-3','R2-0','R2-1','R3-0'])r=finish(r,id);assert.equal(r.champion,'p0');assert.equal(r.status,'complete');assert.equal(r.matches.filter(m=>m.status==='complete').length,7);});
...
test('65th tournament and 21st league entrants are rejected',()=>{assert.throws(()=>reduce(lobby('tournament',64),'extra','P',{type:'join'},1000),/full/);assert.throws(()=>reduce(lobby('league',20),'extra','P',{type:'join'},1000),/full/);});
test('conflicting player reports stop advancement',()=>{let r=reduce(lobby(),'p0','P',{type:'start'},1000);for(const uid of ['p0','p7'])r=reduce(r,uid,uid,{type:'enter',matchId:'R1-0'},1001);r=reduce(r,'p0','P',{type:'result',matchId:'R1-0',winner:'p0',rounds:[2,0],slaps:[0,0]},1002);assert.notEqual(r.matches[0].status,'complete');r=reduce(r,'p7','P',{type:'result',matchId:'R1-0',winner:'p7',rounds:[0,2],slaps:[0,0]},1003);assert.equal(r.matches[0].status,'disputed');assert.deepEqual(r.matches.find(m=>m.id==='R2-0').players,[null,null]);});
test('absence needs a waiting player and an opponent offline for two minutes',()=>{let r=reduce(lobby(),'p0','P',{type:'start'},1000);assert.throws(()=>reduce(r,'p0','P',{type:'abandon',matchId:'R1-0'},130000),/Wait/);r=reduce(r,'p0','P',{type:'enter',matchId:'R1-0'},2000);assert.throws(()=>reduce(r,'p0','P',{type:'abandon',matchId:'R1-0'},3000),/Wait/);r=reduce(r,'p0','P',{type:'abandon',matchId:'R1-0'},130000);assert.deepEqual(r.matches[0].rounds,[2,0]);});
...
test('retrying an agreed result is idempotent',()=>{let r=reduce(lobby(),'p0','P',{type:'start'},1000);r=finish(r,'R1-0');const version=r.version;const next=reduce(r,'p0','P',{type:'result',matchId:'R1-0',winner:'p0',rounds:[3,1],slaps:[4,2]},4000);assert.equal(next.version,version);assert.equal(next.matches.find(m=>m.id==='R2-0').players.filter(Boolean).length,1);});
