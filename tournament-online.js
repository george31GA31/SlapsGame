...
window.onload = function () {
    ...
    // 3. START TOURNAMENT SPECIFIC CONNECTION
    initTournamentMatch();
};
/* ================================
   MULTIPLAYER INIT
...
function initTournamentMatch() {
    // 1. Get Match Data from Bracket
    const role = localStorage.getItem('isf_role'); // 'host' or 'guest'
    const matchCode = localStorage.getItem('isf_code'); // e.g. 'MATCH-L16-1'
    const oppName = localStorage.getItem('isf_tourney_opponent');

    if (!matchCode || !role) {
        alert("Error: Tournament data missing. Returning to bracket.");
    ...
    console.log(`Starting Tournament Match: ${matchCode} as ${role}`);

    // 3. PeerJS Connection (Strict Match ID)
    ...
    gameState.peer.on('error', (err) => {
        ...
        if (err.type === 'unavailable-id') {
            console.log("ID in use. Connection might already be active.");
        }
    });
}
...
function handleNet(msg) {
   ...
   if (msg.type === 'OPPONENT_REJECT') {
    ...
    if (msg.type === 'OPPONENT_FLIP') {
        ...
        if (card && card.element && !card.isFaceUp && gameState.aiHand.filter(c => c.isFaceUp).length < 4) setCardFaceUp(card.element, card, 'ai');
        return;
    }
