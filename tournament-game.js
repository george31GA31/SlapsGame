...
function setCardFaceUp(img, card, owner) {
    ...
    img.src = card.imgSrc; img.classList.remove('card-face-down'); card.isFaceUp = true; card.flipping = false;
...
function showEndGame(titleText, isWin) {
    const modal = document.getElementById('game-message');
    const title = document.getElementById('msg-title');
    const subtitle = document.getElementById('msg-subtitle');
    const btn = document.getElementById('msg-btn');

    // Reset visibility in case they were hidden by previous logic
    if (btn) btn.classList.remove('hidden');
    modal.classList.remove('hidden');

    // 1. CLEAR OLD BUTTONS (Crucial step)
    // We clear the subtitle's innerHTML to remove any old "Rematch" buttons you might have injected previously.
    subtitle.innerHTML = ""; 
    subtitle.style.display = "block"; // Ensure it's visible

    if (isWin) {
        // --- TOURNAMENT WIN LOGIC ---
        title.innerText = "ROUND CLEARED!";
        title.style.color = "#66ff66"; // Green
        subtitle.innerText = "Proceeding to the next stage...";

        // Increment the round number
        let roundIdx = parseInt(localStorage.getItem('isf_tourney_round')) || 0;
        localStorage.setItem('isf_tourney_round', roundIdx + 1);

        // Update Button to go to Next Match
